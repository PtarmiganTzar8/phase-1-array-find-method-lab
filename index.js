...
function superbowlWin(list){
    const object = list.find(function(item){
        return item.result === 'W'})
        if (object){
            return object.year
        }
...
// 1) returns a year the Broncos won
// 2)returns undefined if the record has no wins

// so 1, return the year if result is = to W
// so 2, return undefined if result is not = to W
// function should recieve 1 argument (the array of JavaScripts Object)
//Use find to test each object to see if result is W
//Needs to return the year if W